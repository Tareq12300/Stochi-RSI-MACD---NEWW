...
6) إرسال إشارة شراء عند تحقق جميع الشروط إلى تيليغرام.
...
# نظام الثقة — قابل للتعديل بالكامل من متغيرات Railway
ENABLE_CONFIDENCE_FILTER = (
    os.environ.get("ENABLE_CONFIDENCE_FILTER", "true").lower() == "true"
)
ENABLE_EARLY_ALERTS = (
    os.environ.get("ENABLE_EARLY_ALERTS", "true").lower() == "true"
)
MIN_EARLY_CONFIDENCE = max(0, min(100, int(
    os.environ.get("MIN_EARLY_CONFIDENCE", "75")
)))
MIN_CONFIRMED_CONFIDENCE = max(0, min(100, int(
    os.environ.get("MIN_CONFIRMED_CONFIDENCE", "90")
)))
if MIN_CONFIRMED_CONFIDENCE < MIN_EARLY_CONFIDENCE:
    raise ValueError(
        "MIN_CONFIRMED_CONFIDENCE يجب أن يكون أكبر من أو يساوي MIN_EARLY_CONFIDENCE"
    )
...
def analyze_signal(
    ...
    # الحد الأعلى يُحسب تلقائيًا من مجموع أقصى نقاط كل مجموعة تقييم.
    max_score += 4  # قوة تشبع Stoch RSI
    ...
    max_score += 2  # جودة تقاطع Stoch RSI
    if stoch_crossed:
        score += 2
    ...
    max_score += 4  # أقصى قوة لـ MACD Histogram
    if histogram > 0:
        score += max(
        ...
        max_score += 1  # مكافأة التحول من السالب إلى الموجب
        if macd.get(
            "switched_to_rising"
        ):
            score += 1
    ...
    max_score += 2  # قوة ارتفاع الفوليوم
    if volume_spike:
        score += 2
        reasons.append(
    ...
    max_score += 1  # ملاءمة حركة شمعة الفريم
    if -1.5 <= change_4h <= 2:
        score += 1
        reasons.append(
    ...
    max_score += 2  # ملاءمة الحركة اليومية
    if -10 <= change_24h <= 4:
        score += 2
        reasons.append(
    ...
    max_score += 1  # ملاءمة الحركة الأسبوعية
    if -25 <= change_7d <= 18:
        score += 1
        reasons.append(
    ...
    confidence = int(round(
        max(0.0, min(100.0, score / max(max_score, 1) * 100))
    ))

    if ENABLE_CONFIDENCE_FILTER:
        if confidence >= MIN_CONFIRMED_CONFIDENCE:
            stage = "CONFIRMED"
        elif ENABLE_EARLY_ALERTS and confidence >= MIN_EARLY_CONFIDENCE:
            stage = "EARLY"
        else:
            return None
    else:
    ...
    return {
        ...
        "signal_stage": stage,
        "signal_label": (
            "تنبيه مبكر" if stage == "EARLY" else "إشارة شراء مؤكدة"
        ),
    ...
    try:
        await bot.send_message(
            ...
            text=(
                ...
                + (
                    f"🟡 أقل ثقة مبكرة: *{MIN_EARLY_CONFIDENCE}%*\n"
                    f"🟢 أقل ثقة مؤكدة: *{MIN_CONFIRMED_CONFIDENCE}%*\n"
                    if ENABLE_CONFIDENCE_FILTER
                    else "🎯 فلتر الثقة: *معطّل*\n"
                )
                + f"⏱️ فحص كل "
        ...
        try:
            ...
            for index, coin in enumerate(
                ...
                if signal:
                    ...
                    if stage == "EARLY":
                        if not alert_on_cooldown(
                            coin["symbol"],
                            "EARLY",
                        ):
                            await send_early_alert(bot, signal)
