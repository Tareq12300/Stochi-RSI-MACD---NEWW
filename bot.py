...
5) تحليل الشمعة الحالية أو المغلقة ومقارنتها بالشمعة السابقة في السعر والفوليوم والمؤشرات.
...
# false = تحليل الشمعة الحالية المفتوحة، true = آخر شمعة مغلقة
USE_CLOSED_CANDLE = (
    os.environ.get("USE_CLOSED_CANDLE", "false").strip().lower() == "true"
)

# true = مقارنة شمعة التحليل بالشمعة السابقة مباشرة
COMPARE_WITH_PREVIOUS_CANDLE = (
    os.environ.get("COMPARE_WITH_PREVIOUS_CANDLE", "true").strip().lower()
    == "true"
)

# يُستخدم فقط عند تعطيل المقارنة المباشرة؛ يقارن بمتوسط الشموع السابقة
VOLUME_BASELINE_CANDLES = max(
    int(os.environ.get("VOLUME_BASELINE_CANDLES", "20")), 1
)
...
def calculate_macd(
    ...
    minimum_length = MACD_SLOW + MACD_SIGNAL + 5
    if len(prices) < minimum_length:
        return None

    ema_fast = ema_series(prices, MACD_FAST)
    ema_slow = ema_series(prices, MACD_SLOW)
    ...
    macd_series = [
        ema_fast[index + offset] - ema_slow[index]
        for index in range(len(ema_slow))
    ]

    signal_series = ema_series(
        macd_series, MACD_SIGNAL
    )
...
def detect_volume_spike(
    ...
    """يقارن فوليوم شمعة التحليل بالشمعة السابقة مباشرة أو بمتوسط سابق."""
    ...
    if len(closes) < 2:
        return {
            ...
            "enough_data": False,
            "comparison_mode": "previous_candle",
        }

    # تحويل الفهرس السالب إلى فهرس موجب
    current_pos = analysis_index if analysis_index >= 0 else len(closes) + analysis_index
    if current_pos <= 0 or current_pos >= len(closes):
        return {
            "spike": False,
            "ratio": 0.0,
            "current_volume_usd": 0.0,
    ...
    def volume_usd_at(index: int) -> float:
        if len(quote_volumes) > index:
            value = safe_float(quote_volumes[index])
            if value > 0:
                return value
        if len(base_volumes) > index and len(closes) > index:
            return safe_float(base_volumes[index]) * safe_float(closes[index])
        return 0.0

    current_usd = volume_usd_at(current_pos)

    if COMPARE_WITH_PREVIOUS_CANDLE:
        baseline_usd = volume_usd_at(current_pos - 1)
        comparison_mode = "previous_candle"
    else:
        start_pos = max(0, current_pos - VOLUME_BASELINE_CANDLES)
        previous_values = [
            volume_usd_at(i) for i in range(start_pos, current_pos)
            if volume_usd_at(i) > 0
        ]
        baseline_usd = (
            sum(previous_values) / len(previous_values)
            if previous_values else 0.0
        )
    ...
    return {
        ...
        "previous_volume_usd": baseline_usd,
        "enough_data": current_usd > 0 and baseline_usd > 0,
    ...
    while True:
        try:
            ...
            for index, coin in enumerate(
                ...
                # -1 = الشمعة الحالية المفتوحة، -2 = آخر شمعة مغلقة
                analysis_index = -2 if USE_CLOSED_CANDLE else -1
                previous_index = analysis_index - 1

                minimum_candles = max(
                    43,
                    RSI_PERIOD + STOCH_PERIOD + STOCH_K_SMOOTH
                    + STOCH_D_SMOOTH + 10,
                    MACD_SLOW + MACD_SIGNAL + 5,
                )
                if len(closes) < minimum_candles:
                    await asyncio.sleep(COIN_REQUEST_DELAY)
                    continue

                # نجعل المؤشرات تنتهي عند شمعة التحليل المختارة؛ وبذلك تكون
                # قيم prev هي الشمعة السابقة لها مباشرة.
                indicator_closes = (
                    closes[:analysis_index + 1]
                    if analysis_index != -1
                    else closes
                )

                current_price = closes[analysis_index]
                previous_close = closes[previous_index]
                coin["price"] = current_price
                coin["candle_mode"] = (
                    "closed" if USE_CLOSED_CANDLE else "current"
                )
                ...
                lookback_index = analysis_index - 42
                coin["change_7d"] = (
                    (current_price - closes[lookback_index])
                    / closes[lookback_index] * 100
                    if abs(lookback_index) <= len(closes)
                    and closes[lookback_index]
                    else coin.get("change_7d", 0.0)
                )
                ...
                macd = calculate_macd(indicator_closes)
                volume_data = detect_volume_spike(
                    market_data,
                    analysis_index,
                )
                ...
                coin["volume_comparison_mode"] = (
                    volume_data["comparison_mode"]
                )
